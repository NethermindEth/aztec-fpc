...
export class FpcClient {
  ...
  async createPaymentMethod(input: CreatePaymentMethodInput): Promise<FpcPaymentMethodResult> {
    ...
    const gasLimits = rawGasLimits.add(new Gas(0, L2_GAS_BUFFER));
    ...
    const totalDaGas = BigInt(gasLimits.daGas) + BigInt(teardownGasLimits.daGas);
    const totalL2Gas = BigInt(gasLimits.l2Gas) + BigInt(teardownGasLimits.l2Gas);
