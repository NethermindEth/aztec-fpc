...
function usage() {
  cat <<'EOF'
Usage: bun run e2e:full-lifecycle:local [--help]
...
- FPC_FULL_E2E_NODE_HOST (default 127.0.0.1)
- FPC_FULL_E2E_NODE_PORT (default 8080)
- FPC_FULL_E2E_L1_HOST (default 127.0.0.1)
- FPC_FULL_E2E_L1_PORT (default 8545)
...
function stop_aztec_local_network() {
  ...
  echo "[full-lifecycle-e2e] Stopping aztec local network (pid=$AZTEC_PID)"
...
function cleanup() {
  local node_port="${NODE_PORT:-${FPC_FULL_E2E_NODE_PORT:-8080}}"
  local l1_port="${L1_PORT:-${FPC_FULL_E2E_L1_PORT:-8545}}"
  local attestation_port="${FPC_FULL_E2E_ATTESTATION_PORT:-3300}"
  local topup_ops_port="${FPC_FULL_E2E_TOPUP_OPS_PORT:-3401}"
  ...
  test_cleanup_kill_listener_ports \
    ...
    3000 \
    3001
  test_cleanup_reset_state "[full-lifecycle-e2e]" "$REPO_ROOT"
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
for cmd in aztec bun node; do
  if ! command -v "$cmd" >/dev/null 2>&1; then
    echo "[full-lifecycle-e2e] ERROR: missing required command '$cmd'" >&2
    exit 1
  fi
done
...
NODE_HOST="${FPC_FULL_E2E_NODE_HOST:-127.0.0.1}"
NODE_PORT="${FPC_FULL_E2E_NODE_PORT:-8080}"
L1_HOST="${FPC_FULL_E2E_L1_HOST:-127.0.0.1}"
L1_PORT="${FPC_FULL_E2E_L1_PORT:-8545}"
START_LOCAL_NETWORK="${FPC_FULL_E2E_START_LOCAL_NETWORK:-1}"
if [[ "$START_LOCAL_NETWORK" != "0" && "$START_LOCAL_NETWORK" != "1" ]]; then
  echo "[full-lifecycle-e2e] ERROR: FPC_FULL_E2E_START_LOCAL_NETWORK must be 0 or 1, got '$START_LOCAL_NETWORK'" >&2
  exit 1
fi
...
if [[ "$START_LOCAL_NETWORK" == "1" ]]; then
  if [[ "$NODE_RUNNING" -ne "$L1_RUNNING" ]]; then
    ...
    echo "[full-lifecycle-e2e] ERROR: stop the stale process or ensure both endpoints are reachable before rerunning" >&2
    exit 1
  fi
  if [[ "$NODE_RUNNING" -eq 1 && "$L1_RUNNING" -eq 1 ]]; then
    echo "[full-lifecycle-e2e] Reusing existing local aztec devnet ($NODE_HOST:$NODE_PORT) and anvil ($L1_HOST:$L1_PORT)"
  else
    if [[ "$NODE_HOST" != "127.0.0.1" || "$NODE_PORT" != "8080" || "$L1_HOST" != "127.0.0.1" || "$L1_PORT" != "8545" ]]; then
      echo "[full-lifecycle-e2e] ERROR: auto-start only supports default endpoints (127.0.0.1:8080, 127.0.0.1:8545)" >&2
      echo "[full-lifecycle-e2e] ERROR: pre-start your custom endpoints and set FPC_FULL_E2E_START_LOCAL_NETWORK=0" >&2
      exit 1
    fi
    echo "[full-lifecycle-e2e] Starting aztec local network in background"
    ...
    if ! wait_for_port "$NODE_HOST" "$NODE_PORT" 180; then
      echo "[full-lifecycle-e2e] ERROR: aztec node did not become reachable on $NODE_HOST:$NODE_PORT" >&2
      tail -n 200 "$TMP_DIR/aztec-local-network.log" >&2 || true
      exit 1
    fi
    if ! wait_for_port "$L1_HOST" "$L1_PORT" 180; then
      echo "[full-lifecycle-e2e] ERROR: anvil did not become reachable on $L1_HOST:$L1_PORT" >&2
      tail -n 200 "$TMP_DIR/aztec-local-network.log" >&2 || true
      exit 1
    fi
  fi
