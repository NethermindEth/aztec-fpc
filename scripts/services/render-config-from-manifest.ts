...
function parsePort(value: string, fieldName: string): number {
  ...
  if (parsed > 65535) {
    throw new CliError(`Invalid ${fieldName}: expected value in range 1..65535`);
  }
  return parsed;
}
...
function parseFeeBips(value: string, fieldName: string): number {
  const normalized = value.trim();
  if (!UINT_DECIMAL_PATTERN.test(normalized)) {
    throw new CliError(`Invalid ${fieldName}: expected integer in range 0..10000`);
  }
  const parsed = Number(normalized);
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--operator-secret-provider":
        operatorSecretProvider = parseSecretProvider(nextArg(argv, i, arg), arg);
        i += 1;
        break;
      ...
      case "--topup-confirm-timeout-ms":
        topupConfirmTimeoutMs = parsePositiveInteger(nextArg(argv, i, arg), arg);
        i += 1;
        break;
      ...
      case "--topup-confirm-poll-max-ms":
        topupConfirmPollMaxMs = parsePositiveInteger(nextArg(argv, i, arg), arg);
        i += 1;
        break;
      case "--l1-operator-secret-provider":
        l1OperatorSecretProvider = parseSecretProvider(nextArg(argv, i, arg), arg);
        i += 1;
        break;
...
function resolveAttestationSecretMaterial(
  ...
  if (!operatorMatchesDeployer && !args.operatorSecretKey && !args.operatorSecretRef) {
    throw new CliError(
