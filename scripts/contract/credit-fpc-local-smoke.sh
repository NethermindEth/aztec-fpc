...
function stop_aztec_local_network() {
  ...
  echo "[credit-smoke] Stopping aztec local network (pid=$AZTEC_PID)"
...
RESET_LOCAL_STATE="${CREDIT_FPC_SMOKE_RESET_LOCAL_STATE:-}"
if [[ -z "$RESET_LOCAL_STATE" ]]; then
  RESET_LOCAL_STATE="1"
fi
if [[ "$RESET_LOCAL_STATE" != "0" && "$RESET_LOCAL_STATE" != "1" ]]; then
  echo "[credit-smoke] ERROR: CREDIT_FPC_SMOKE_RESET_LOCAL_STATE must be 0 or 1, got '$RESET_LOCAL_STATE'" >&2
  exit 1
fi
...
if [[ "$START_LOCAL_NETWORK" == "1" ]]; then
  if [[ "$NODE_RUNNING" -eq 1 && "$L1_RUNNING" -eq 1 ]]; then
    echo "[credit-smoke] Reusing existing local aztec devnet ($NODE_HOST:$NODE_PORT) and anvil ($L1_HOST:$L1_PORT)"
  else
    echo "[credit-smoke] Starting aztec local network in background"
    ...
    if ! wait_for_port "$NODE_HOST" "$NODE_PORT" 180; then
      echo "[credit-smoke] ERROR: aztec node did not become reachable on $NODE_HOST:$NODE_PORT" >&2
      tail -n 200 "$TMP_DIR/aztec-local-network.log" >&2 || true
      exit 1
    fi
    if ! wait_for_port "$L1_HOST" "$L1_PORT" 180; then
      echo "[credit-smoke] ERROR: anvil did not become reachable on $L1_HOST:$L1_PORT" >&2
      tail -n 200 "$TMP_DIR/aztec-local-network.log" >&2 || true
      exit 1
    fi
  fi
  ...
  if [[ "$NODE_RUNNING" -ne 1 || "$L1_RUNNING" -ne 1 ]]; then
    echo "[credit-smoke] ERROR: local network auto-start disabled, but $NODE_HOST:$NODE_PORT or $L1_HOST:$L1_PORT is not reachable" >&2
    exit 1
  fi
fi
