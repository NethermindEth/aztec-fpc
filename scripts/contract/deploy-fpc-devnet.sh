...
if [[ "$MODE" == "local" ]]; then
  ...
  if [[ "$START_LOCAL_NETWORK" == "1" ]]; then
    # ── Bare-metal: manage local network lifecycle ──
    ...
    # ── Docker/external: network is already running ──
    NODE_URL="${AZTEC_NODE_URL:-http://${NODE_HOST}:${NODE_PORT}}"
    echo "[deploy-fpc-local] Using external network at $NODE_URL (START_LOCAL_NETWORK=0)"
  fi
