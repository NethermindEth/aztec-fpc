...
OPERATOR_SECRET_KEY_REF="${FPC_OPERATOR_SECRET_KEY_REF:-}"
...
if [[ -n "${OPERATOR_SECRET_KEY}" && -n "${OPERATOR_SECRET_KEY_REF}" ]]; then
  echo "ERROR: Set only one of FPC_OPERATOR_SECRET_KEY or FPC_OPERATOR_SECRET_KEY_REF" >&2
  exit 1
fi
