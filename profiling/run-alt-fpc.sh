#!/usr/bin/env bash
# Compile contracts, deploy, and profile AltFPC gate counts (pay_and_mint + pay_fee).
#
# Run ./profiling/setup.sh once first, then re-run this after every contract change.
#
# Usage:
#   ./profiling/run-alt-fpc.sh
#
# Environment:
...
echo ""
echo "[profile] Running AltFPC gate count profiler..."
AZTEC_NODE_URL="$NODE_URL" node "$SCRIPT_DIR/profile-alt-fpc.mjs"
