...
class PayAndMintPaymentMethod {
  constructor(fpcAddress, transferAuthWit, transferNonce, rateNum, rateDen, validUntil, mintAmount, gasSettings) {
...
async function main() {
  ...
  // registerContract only registers artifact/instance; it does NOT enable
  // private-log scanning for notes created by the contract.
  ...
  // ── Quote authwit (operator authorises the rate for this user) ─────────────
  ...
  const sendPayAndMint = new PayAndMintPaymentMethod(
    fpcAddress, sendTransferAuthWit, SEND_NONCE,
  ...
  // balance_of().simulate() triggers PXE sync, so this also catches late sync.
  ...
  if (creditBalance < payFeeTeardownCost) {
    ...
    // Use a different valid_until to produce a distinct authwit nullifier
    const DEV_VALID_UNTIL = VALID_UNTIL + 5n;
    const devQuoteInnerHash = await computeInnerAuthWitHash([
      new Fr(QUOTE_DOMAIN_SEP),
      fpcAddress.toField(),
      tokenAddress.toField(),
      new Fr(RATE_NUM),
      new Fr(RATE_DEN),
      new Fr(DEV_VALID_UNTIL),
      userAddress.toField(),
    ]);
    const devQuoteAuthWit = await wallet.createAuthWit(operatorAddress, {
      consumer: fpcAddress,
      innerHash: devQuoteInnerHash,
    });

    const devPayAndMint = new PayAndMintPaymentMethod(
      fpcAddress, devTransferAuthWit, DEV_NONCE,
  ...
  // Fresh quote authwit for profiling (different valid_until to avoid nullifier collision)
  const PROFILE_VALID_UNTIL = VALID_UNTIL + 10n;
  const profileQuoteInnerHash = await computeInnerAuthWitHash([
    new Fr(QUOTE_DOMAIN_SEP),
    fpcAddress.toField(),
    tokenAddress.toField(),
    new Fr(RATE_NUM),
    new Fr(RATE_DEN),
    new Fr(PROFILE_VALID_UNTIL),
    userAddress.toField(),
  ]);
  const profileQuoteAuthWit = await wallet.createAuthWit(operatorAddress, {
    consumer: fpcAddress,
    innerHash: profileQuoteInnerHash,
  });

  const payAndMintPayment = new PayAndMintPaymentMethod(
    fpcAddress, profileTransferAuthWit, PROFILE_NONCE,
  ...
  const payAndMintResult = await tokenAsUser.methods
    ...
    .profile({
      ...
      from: userAddress,
      additionalScopes: [operatorAddress],
      authWitnesses: [profileQuoteAuthWit],
      profileMode: 'full',
