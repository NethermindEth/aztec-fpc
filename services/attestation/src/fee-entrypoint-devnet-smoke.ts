...
function loadArtifact(artifactPath: string): ContractArtifact {
  ...
  try {
    ...
    if (err instanceof Error && err.message.includes("Contract's public bytecode has not been transpiled")) {
...
function getConfig(): SmokeConfig {
  ...
  const l1PrivateKey = (process.env.FPC_SMOKE_L1_PRIVATE_KEY ?? DEFAULT_LOCAL_L1_PRIVATE_KEY) as Hex;
...
async function topUpFpcFeeJuice(
  ...
  await feeJuice.methods
    .claim(AztecAddress.fromString(fpcAddress), topupWei, claimSecret, new Fr(messageLeafIndex))
    .send({ from: operator });
...
async function main() {
  ...
  const maxGasCostNoTeardown = BigInt(config.daGasLimit) * feePerDaGas + BigInt(config.l2GasLimit) * feePerL2Gas;
  ...
  const fpc = await Contract.deploy(wallet, fpcArtifact, [operator, token.address]).send({ from: operator });
  ...
  const transferCall = token.methods.transfer_in_private(user, operator, expectedCharge, transferAuthwitNonce);
  const transferAuthwit = await wallet.createAuthWit(user, {
    caller: fpc.address,
    call: await transferCall.getFunctionCall(),
  });

  const userBefore = BigInt((await token.methods.balance_of_private(user).simulate({ from: user })).toString());
  ...
  const feeEntrypointCall = await fpc.methods
    .fee_entrypoint(transferAuthwitNonce, config.rateNum, config.rateDen, validUntil)
    .getFunctionCall();
  const fpcPaymentMethod = {
    getAsset: async () => ProtocolContractAddress.FeeJuice,
    getExecutionPayload: async () =>
      new ExecutionPayload([feeEntrypointCall], [quoteAuthwit, transferAuthwit], [], [], fpc.address),
  ...
  const userAfter = BigInt((await token.methods.balance_of_private(user).simulate({ from: user })).toString());
  const operatorAfter = BigInt((await token.methods.balance_of_private(operator).simulate({ from: operator })).toString());
