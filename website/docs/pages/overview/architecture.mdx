...
|------|----|-----------|----------|
| Wallet/SDK | Attestation | HTTP REST | `GET /quote`, `GET /accepted-assets` |
| Wallet/SDK | FPC Contract | Aztec tx | `fee_entrypoint()`, `cold_start_entrypoint()` |
| Top-up | L1 Portal | Ethereum tx | Bridge Fee Juice |
| Top-up | Aztec L2 | Aztec tx | Claim bridged tokens |
| Attestation | Aztec Node | PXE RPC | Gas estimation, balance queries |
| FPC Contract | Token Contract | Aztec call | `transfer_in_public()`, `transfer_to_public()` |
...
:::tip
For production deployments, use **KMS or HSM** for operator keys. See [Security Model](/overview/security) for details.
:::
