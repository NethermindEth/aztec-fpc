...
- Integrating against the Nethermind-operated testnet? → **[Testnet Deployment](/reference/testnet-deployment)**
- Integrating against a custom operator? → Resolve via [`.well-known/fpc.json`](/services/attestation#wallet-discovery) using the key `(network_id, asset_address, fpc_address)`.
...
- [SDK — Getting Started](/sdk/getting-started) — full SDK context including the standard flow
- [SDK — API Reference](/sdk/api-reference#executecoldstart) — complete type signatures
- [Quote System](/overview/quote-system#cold-start) — the domain separator, 9-field preimage, and replay protection
- [Architecture](/overview/architecture#cold-start) — data-flow diagram across L1, attestation, and FPC
